...
QString QSQLiteDriver::escapeIdentifier(const QString &identifier, IdentifierType type) const
{
    ...
    if (indexOfSeparator > -1) {
        ...
        if (leftEnclose.isEmpty() || rightEnclose.isEmpty())
            return leftEnclose.toString() + leftName.toString() + leftEnclose.toString()
                   + u'.' + rightEnclose.toString() + rightName.toString()
                   + rightEnclose.toString();
    }
...
QString QSQLiteDriver::stripDelimiters(const QString &identifier, IdentifierType type) const
{
    ...
    if (indexOfSeparator > -1) {
        ...
        if (leftEscaped || rightEscaped) {
            ...
            return leftName.toString() + u'.' + rightName.toString();
        }
    }
